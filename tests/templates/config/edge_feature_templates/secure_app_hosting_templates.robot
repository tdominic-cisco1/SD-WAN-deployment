...
Get Secure App Hosting Feature Templates
    ...
    ${r}=    Json Search List    ${r.json()}    data[?templateType=='virtual-application-utd']
...
Verify Edge Feature Template Secure App Hosting Feature Template {{ ft_yaml.name }}
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    "virtual-application".vipValue[0].utd.nat
    ...    {{ ft_yaml.nat | default("not_defined") }}
    ...    {{ ft_yaml.nat_variable | default("not_defined") }}
    ...    msg=nat

    Should Be Equal Value Json Yaml UX1    ${ft.json()}    "virtual-application".vipValue[0].utd."database-url"
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    "virtual-application".vipValue[0].utd."resource-profile"
