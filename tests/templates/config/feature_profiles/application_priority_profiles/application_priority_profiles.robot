*** Settings ***
Documentation   Verify Application Priority Feature Profile Configuration
Name            Application Priority Profiles Summary
Suite Setup     Login SDWAN Manager
Suite Teardown  Run On Last Process   Logout SDWAN Manager
Default Tags    sdwan    config    feature_profiles    application_priority_profiles
...
Verify Feature Profiles Application Priority Profiles {{ profile.name }}
 ...
 {% if 'strict_config_check' not in robot_exclude_tags | default() %}
    ${profile_features_res}=   GET On Session With Retry    sdwan_manager    /dataservice/v1/feature-profile/sdwan/application-priority/${profile_id}
    ...
    {% for key,value in profile.items() if key != 'name' and key != 'description' %}
        ...
        {% endfor %}
    {% endfor %}
        Append To List    ${expected_features}    {{ profile.name }}_settings
