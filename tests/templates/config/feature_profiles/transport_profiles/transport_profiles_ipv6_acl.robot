...
Verify Feature Profiles Transport Profiles {{ profile.name }} IPv6 ACL Feature
    ...
    ${transport_ipv6_acl_res}=    GET On Session With Retry    sdwan_manager    /dataservice/v1/feature-profile/sdwan/transport/${profile_id}/ipv6-acl
    Set Suite Variable    ${transport_ipv6_acl_res}
    ${transport_acl}=    Json Search List    ${transport_ipv6_acl_res.json()}    data[].payload
...
{% for acl in profile.ipv6_acls | default([]) %}
    ...
    Run Keyword If    $acl_feature is None    Fail    IPv6 ACL feature '{{ acl.name }}' expected in transport profile '{{ profile.name }}'
...
{% for sequence in acl.sequences | default([]) %}
    ...
    Should Be Equal Value Json Yaml    ${sequence}    sequenceName    {{ sequence.name | default(defaults.sdwan.feature_profiles.transport_profiles.ipv6_acls.sequences.name ) }}    not_defined    msg=acl.sequence.name
...
{% endfor %}
    ...
    Should Be Equal Value Json Yaml    ${sequence}    matchEntries[0].icmpMsg    {{ sequence.match_entries.icmpv6_messages | default('not_defined') }}    not_defined    msg=acl.sequence.match_entries.icmpv6_messages
