...
Verify Feature Profiles Transport Profiles {{ profile.name }} Route Policy Feature
    ...
    ${transport_route_policy_res}=    GET On Session With Retry    sdwan_manager    /dataservice/v1/feature-profile/sdwan/transport/${profile_id}/route-policy
    Set Suite Variable    ${transport_route_policy_res}
    ${transport_route_policy}=    Json Search List    ${transport_route_policy_res.json()}    data[].payload
...
{% for route_policy in profile.route_policies | default([]) %}
    ...
    ${transport_route_policies_{{ route_policy.name }}}=    Json Search    ${transport_route_policy}    [?name=='{{ route_policy.name }}'] | [0]
    Run Keyword If    $transport_route_policies_{{ route_policy.name }} is None    Fail    Route Policy feature '{{ route_policy.name }}' expected in transport profile '{{ profile.name }}'
    ...
    Should Be Equal Value Json Yaml    ${transport_route_policies_{{ route_policy.name }}}    data.defaultAction    {{ route_policy.default_action }}    not_defined    msg=route_policy.default_action

    Should Be Equal Value Json List Length    ${transport_route_policies_{{ route_policy.name }}}    data.sequences    {{ route_policy.get('sequences', []) | length }}    msg=sequences length
{% if route_policy.get('sequences', []) | length > 0 %}
    Log    === Sequences List ===
{% for sequence in route_policy.sequences | default([]) %}
    Log    === Sequence {{ sequence.id }} ===
    ${sequence}=    Json Search    ${transport_route_policies_{{ route_policy.name }}}    data.sequences[?sequenceId.value==`{{ sequence.id }}`] | [0]
    ...
    Should Be Equal Value Json Yaml    ${sequence}    sequenceName    {{ sequence.name | default(defaults.sdwan.feature_profiles.transport_profiles.route_policies.sequences.name ) }}    not_defined    msg=route_policy.sequence.name
