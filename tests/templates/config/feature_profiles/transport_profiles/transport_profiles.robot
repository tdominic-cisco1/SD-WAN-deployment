...
Default Tags    sdwan    config    feature_profiles    transport_profiles
...
Verify Feature Profiles Transport Profile {{ profile.name }}
 ...
 {% if 'strict_config_check' not in robot_exclude_tags | default() %}
    ${profile_features_res}=   GET On Session With Retry    sdwan_manager    /dataservice/v1/feature-profile/sdwan/transport/${profile_id}
