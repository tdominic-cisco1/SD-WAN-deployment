...
Default Tags    sdwan    config    feature_profiles    transport_profiles    ipv4_trackers    ipv4_tracker_groups
...
Verify Feature Profiles Transport Profiles {{ profile.name }} IPv4 Tracker Feature
    ...
    ${transport_ipv4_tracker_res}=    GET On Session With Retry    sdwan_manager    /dataservice/v1/feature-profile/sdwan/transport/${profile_id}/tracker
    Set Suite Variable    ${transport_ipv4_tracker_res}
    ${transport_ipv4_tracker}=    Json Search List    ${transport_ipv4_tracker_res.json()}    data[].payload
...
{% for tracker in profile.ipv4_trackers | default([]) %}
    ...
    ${tracker_feature}=    Json Search    ${transport_ipv4_tracker}    [?name=='{{ tracker.name }}'] | [0]
    Run Keyword If    $tracker_feature is None    Fail    IPv4 tracker feature '{{ tracker.name }}' expected in transport profile '{{ profile.name }}'
    ...
    Should Be Equal Value Json Yaml    ${tracker_feature}    data.endpointTrackerType    {{ tracker.endpoint_tracker_type | replace("icmp", "interface-icmp") }}    {{ tracker.endpoint_tracker_type_variable | default('not_defined') }}    msg=endpoint_tracker_type
    ...
    {% else %}
    Should Be Equal Value Json Yaml    ${tracker_feature}    data.endpointTrackerType    {{ tracker.endpoint_tracker_type | default('not_defined') | replace("http", "interface") }}    {{ tracker.endpoint_tracker_type_variable | default('not_defined') }}    msg=endpoint_tracker_type
...
Verify Feature Profiles Transport Profiles {{ profile.name }} IPv4 Tracker Group Feature
    Run Keyword If    $profile is None    Fail    Feature Profile '{{ profile.name }}' should be present on the Manager
    ${transport_tracker_grp_res}=    GET On Session With Retry    sdwan_manager    /dataservice/v1/feature-profile/sdwan/transport/${profile_id}/trackergroup
...
{% for tracker_grp in profile.ipv4_tracker_groups | default([]) %}
    ...
    Run Keyword If    $tracker_grp_feature is None    Fail    IPv4 tracker group feature '{{ tracker_grp.name }}' expected in transport profile '{{ profile.name }}'
    ...
    ${transport_tracker_data}=    Json Search List    ${transport_ipv4_tracker_res.json()}    data
