...
Get IPv6 Data Prefix Lists
    ...
    ${ipv6_data_prefix_raw}=    GET On Session With Retry    sdwan_manager    /dataservice/v1/feature-profile/sdwan/policy-object/${profile_id}/data-ipv6-prefix
...
Verify Feature Profiles Policy Object Profile {{ sdwan.feature_profiles.policy_object_profile.name }} IPv6 Data Prefix List Feature {{ ipv6_data_prefix_list.name }}
    ...
    ${ipv6_data_prefix_list}=    Json Search    ${ipv6_data_prefix_raw.json()}    data[?payload.name=='{{ ipv6_data_prefix_list.name }}'] | [0].payload
    Run Keyword If    $ipv6_data_prefix_list is None    Fail    Feature '{{ ipv6_data_prefix_list.name }}' expected to be configured within the policy object profile '{{ sdwan.feature_profiles.policy_object_profile.name }}' on the Manager

    Should Be Equal Value Json String    ${ipv6_data_prefix_list}    name    {{ ipv6_data_prefix_list.name }}    msg=name
    Should Be Equal Value Json Special_String    ${ipv6_data_prefix_list}    description    {{ ipv6_data_prefix_list.description | default('not_defined') | normalize_special_string }}    msg=description

    ${prefixes_list}=    Create List    {{ ipv6_data_prefix_list.get('prefixes', []) | join('   ') }}
    # Extract features from the JSON
    ${prefixes_list_json}=    Evaluate    [f"{item['ipv6Address']['value']}/{item['ipv6PrefixLength']['value']}" for item in ${ipv6_data_prefix_list['data']['entries']} if 'ipv6Address' in item and 'ipv6PrefixLength' in item]
