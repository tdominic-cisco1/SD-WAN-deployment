*** Settings ***
Documentation   Verify Service Feature Profile Configuration OSPF Features
Name            Service Profiles OSPF Features
Suite Setup     Login SDWAN Manager
Suite Teardown  Run On Last Process    Logout SDWAN Manager
Default Tags    sdwan    config    feature_profiles    service_profiles    ospf
...
Verify Feature Profiles Service Profiles {{ profile.name }} OSPF Feature
    ...
    ${service_ospf_res}=    GET On Session With Retry    sdwan_manager    /dataservice/v1/feature-profile/sdwan/service/${profile_id}/routing/ospf
    Set Suite Variable    ${service_ospf_res}
    ${service_ospf}=    Json Search List    ${service_ospf_res.json()}    data[].payload
    Run Keyword If    ${service_ospf} == []    Fail    OSPF feature(s) expected to be configured within the service profile '{{ profile.name }}' on the Manager
...
{% for ospf in profile.ospf_features | default([]) %}
    ...
    ${ospf_feature}=    Json Search    ${service_ospf}    [?name=='{{ ospf.name }}'] | [0]
    Run Keyword If    $ospf_feature is None    Fail    OSPF feature '{{ ospf.name }}' expected in service profile '{{ profile.name }}'
...
{% for redistribute in ospf.redistributes | default([]) %}
    ...
    Should Be Equal Value Json Yaml    ${ospf_feature}    data.redistribute[{{ loop.index0 }}].dia    {{ redistribute.dia | default('not_defined') }}    {{ redistribute.dia_variable | default('not_defined') }}    msg=ospf.redistributes.dia
    Should Be Equal Value Json Yaml    ${ospf_feature}    data.redistribute[{{ loop.index0 }}].translateRibMetric    {{ redistribute.translate_rib_metric | default('not_defined') }}    {{ redistribute.translate_rib_metric_variable | default('not_defined') }}    msg=ospf.redistributes.translate_rib_metric
